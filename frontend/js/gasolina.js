...
import { ensureTasas, fillTasaSelect, getRateLabel } from "./tasas.js";
...
export async function loadGasolina() {
  ...
  if (!ventas.length) {
    tbody.innerHTML = renderEmptyRow(7, "No hay ventas de gasolina registradas.");
    return;
  }
...
export function initGasolina() {
  ...
  ventaForm.addEventListener("submit", async (event) => {
    ...
    const payload = {
      ...
      tasa_cambio_id: Number(formData.get("tasa_cambio_id")),
    ...
    try {
      ...
      ventaForm.monto_recibido_oro.value = "0.000";
      ventaForm.monto_recibido_reales.value = "0.00";
      fillTasaSelect("gasolina-tasa");
